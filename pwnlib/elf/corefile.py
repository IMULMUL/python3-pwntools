...
class Core(ELF):
            ...
            for segment in self.segments:
                ...
                for note in iter_notes(segment):
                    ...
                    if note.n_descsz == ctypes.sizeof(prstatus_type) \
                            and note.n_type == 'NT_GNU_ABI_TAG':
    ...
    def _parse_nt_file(self, note):
        ...
        for i in range(count):
            filename = t.recvuntil(b'\x00', drop=True).decode('utf8', 'surrogateescape')
            start = starts[i]
    ...
    def getenv(self, name):
        ...
        return self.string(self.env[name]).split(b'=', 1)[-1]
