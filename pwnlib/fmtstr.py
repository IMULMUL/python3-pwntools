...
def fmtstr_payload(offset, writes, numbwritten=0, write_size='byte'):
    r"""fmtstr_payload(offset, writes, numbwritten=0, write_size='byte') -> bytes
    ...
    for where, what in writes.items():
        for i in range(0, number):
            ...
            if to_add != 0:
                payload += "%{}c".format(to_add).encode('utf8')

            payload += "%{}${}n".format(offset + fmtCount, formatz).encode('utf8')
...
class FmtStr:
    ...
    def leak_stack(self, offset, prefix=b''):
        leak = self.execute_fmt(prefix + "START%{}$pEND".format(offset).encode('utf8'))
        try:
