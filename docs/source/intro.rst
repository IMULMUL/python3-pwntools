...
unpacking codes, and littering your code with helper routines.
    ...
    True
    >>> leet = codecs.decode('37130000', 'hex_codec')
...
from the internet!  The :mod:`pwnlib.asm` module is full of awesome.
    ...
    >>> codecs.encode(asm('mov eax, 0'), 'hex_codec')
    b'b800000000'

But if you do, it's easy to suss out!

    >>> print(disasm(codecs.decode('6a0258cd80ebf9', 'hex_codec')))
       0:   6a 02                   push   0x2
       2:   58                      pop    eax
       3:   cd 80                   int    0x80
       5:   eb f9                   jmp    0x0
...
file descriptor 4 to `stdin`, `stdout`, and `stderr`, and then pop a shell!
    ...
    >>> codecs.encode(asm(shellcraft.setreuid() + shellcraft.dupsh(4)), 'hex_codec')
